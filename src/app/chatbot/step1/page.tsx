...
function getAssistantAnswer(q: string): AnswerFormat {
  ...
  return {
    summary: "임대차 보증금을 돌려받지 못한 상황으로, 이는 임대차 계약에 따른 보증금 반환 문제로 인해 발생한 것으로 이해하겠습니다.",
    law: "관련 법률인 '임대차 보증금 반환'에 관한 법률을 확인하여 보증금 반환 절차 및 조건을 파악하고, 이를 토대로 상대방과 협의해 보증금 반환을 요구할 수 있습니다.",
    caseExample: "판례를 통해 유사한 상황에서 어떻게 보증금 반환이 이루어졌는지 확인하고, 해당 사례를 참고하여 대응 방안을 모색할 수 있습니다.",
            ...
            <div key={i} className="flex items-start justify-start gap-1 mt-6">
              ...
              <div className="bg-violet-100/70 p-8 mt-12 ml-[-14px] rounded-tr-[30px] rounded-br-[30px] rounded-bl-[30px] rounded-tl-none max-w-[85%] space-y-3 text-[1.1rem]">
                ...
                <div>
                  ...
                  <div className="mt-2">
                    <div className="mb-2"><span className="font-semibold">법률 해결방안:</span><br />{(m.text as AnswerFormat).law}</div>
                    <div className="mb-2"><span className="font-semibold">유사 사례:</span><br />{(m.text as AnswerFormat).caseExample}</div>
                    <div><span className="font-semibold">용어 설명:</span><br />{(m.text as AnswerFormat).term}</div>
                  </div>
                </div>
              </div>
            </div>
